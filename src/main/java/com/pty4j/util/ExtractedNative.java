...
class ExtractedNative {
  ...
  static final String DEFAULT_RESOURCE_NAME_PREFIX = "os/";
  ...
  private void init() {
    try {
      myResourceOsArchSubPath = ObjUtil.defaultIfNull(myResourceOsArchSubPath, PtyUtil.getNativeLibraryOsArchSubPath());
      myResourceNamePrefix = ObjUtil.defaultIfNull(myResourceNamePrefix, DEFAULT_RESOURCE_NAME_PREFIX);
      synchronized (this) {
        if (!myInitialized) {
          doInit();
        }
